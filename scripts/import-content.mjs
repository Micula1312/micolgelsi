...
const SECTION_NAMES=new Set(['TITLE','AUTHOR','TEASER','AVATAR','FEATURED','START','END','STATUS','TYPE','MEDIUM','RESEARCH','WITH','COLLABORATORS','MAIN LINK','ARTISTS INVOLVED','CURATED BY','CREDITS','PHOTO CREDITS','EXCERPT','DESCRIPTION','SHORT','CRITICAL TEXTS','PUBLICATIONS','COMMUNICATION','LINKS','MOMENTS','OUTPUTS','PARENT','WORKS']);
...
const parseReferences=(value='')=>String(value).replace(/\r/g,'').split('\n').map(line=>line.trim()).filter(Boolean).map(line=>{const parts=line.split('|');if(parts.length>1){const url=parts.pop().trim();return{label:parts.join('|').trim(),url:normalizeUrl(url)};}return{label:'',url:normalizeUrl(line)};}).filter(item=>item.url);
...
const resolveAvatar=async(mediaDir,baseUrl,explicit='')=>{
  ...
  if(raw){if(/^https?:\/\//i.test(raw)||raw.startsWith('/'))return raw;try{await fs.access(path.join(mediaDir,raw));return`${baseUrl}/${raw.split(path.sep).join('/')}`;}catch{}}
...
const generateMarkdown=async({slug,sourceKind,data})=>{
  ...
  const videoSingular=await listMedia(path.join(mediaDir,'video'),VIDEO_EXT),videoPlural=await listMedia(path.join(mediaDir,'videos'),VIDEO_EXT);
  const videos=[...new Set([...videoSingular.map(n=>`${baseUrl}/video/${n}`),...videoPlural.map(n=>`${baseUrl}/videos/${n}`)])];
  const criticalTexts=parseReferences(data['CRITICAL TEXTS']),publications=parseReferences(data.PUBLICATIONS),communication=parseReferences(data.COMMUNICATION),links=splitList(data.LINKS).map(normalizeUrl);
  ...
  const excerpt=(data.EXCERPT||data.SHORT||'').trim(),description=(data.DESCRIPTION||(!data.EXCERPT?.trim()?data.SHORT:'')||'').trim();
  ...
  if(collection!=='external'){
    for(const[key,values]of[['gallery',gallery],['videos',videos]]){front.push(`${key}:`);values.length?values.forEach(x=>front.push(`  - ${yamlString(x)}`)):front.push('  []');}
    for(const[key,values]of[['criticalTexts',criticalTexts],['publications',publications],['communication',communication]]){front.push(`${key}:`);if(values.length){values.forEach(item=>{front.push(`  - label: ${yamlString(item.label)}`,`    url: ${yamlString(item.url)}`);});}else front.push('  []');}
    front.push('links:');links.length?links.forEach(x=>front.push(`  - ${yamlString(x)}`)):front.push('  []');
  }
